...
runProcess('full', df_full, df_full)

selected <- df_full %>%
  filter(x_nyomda == 'Heltai nyomda')
runProcess('heltai', df_full, selected)

selected <- df_full %>%
  filter(x_nyelvek == 'latin')
runProcess('latin', df_full, selected)

runProcess('1971', df_1971, df_1971)

dirName <- 'data_raw/v03/abundance/'
df_full %>% select(count = x_letezo_peldanyok_szama) %>% write_csv(sprintf('%s/%s_basis.csv', dirName, '2024'))
df_1971 %>% select(count = x_letezo_peldanyok_szama) %>% write_csv(sprintf('%s/%s_basis.csv', dirName, '1971'))
